...
1. **Worker** thăm dò mỗi kết nối đích theo một khoảng thời gian có thể cấu hình (`WORKER_INTERVAL_SECS`), đọc `pg_stat_statements` hoặc `performance_schema` và lưu trữ các Snapshot vào cơ sở dữ liệu metadata.
...
| `internal/adapter/` | Interface `DBAnalyzer` + adapter PostgreSQL, MySQL, MariaDB (truy vấn chậm, EXPLAIN, chỉ mục) |
...
Tất cả logic dành riêng cho cơ sở dữ liệu đều nằm sau interface `DBAnalyzer` trong `internal/adapter/`. Hàm factory `NewAdapter(dbType string)` trả về implementation đúng (PostgreSQL, MySQL hoặc MariaDB). Thêm hỗ trợ loại cơ sở dữ liệu mới nghĩa là tạo các file adapter thỏa mãn interface — không cần thay đổi API hay Worker.
