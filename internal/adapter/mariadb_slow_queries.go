...
// GetSlowQueries fetches top slow queries from MariaDB performance_schema.
func (a *MariaDBAdapter) GetSlowQueries(ctx context.Context, opts QueryOpts) ([]models.SlowQuery, error) {
	limit := 50
	if opts.Limit > 0 {
		limit = opts.Limit
	}
	...
	if err != nil {
		return nil, fmt.Errorf("mariadb slow queries: %w", err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&q.QueryID, &q.Query, &q.Calls, &q.TotalExecMs, &q.MeanExecMs, &q.Rows); err != nil {
			return nil, fmt.Errorf("scan mariadb slow query: %w", err)
		}
		q.SnapshotAt = now
