...
// GetIndexStats returns unused indexes from MariaDB via performance_schema.
func (a *MariaDBAdapter) GetIndexStats(ctx context.Context) ([]models.IndexStat, error) {
	...
	`)
	if err != nil {
		return nil, fmt.Errorf("mariadb index stats: %w", err)
	}
	defer rows.Close()
...
func (a *MariaDBAdapter) GetDuplicateIndexes(ctx context.Context) ([]models.DuplicateIndex, error) {
	rows, err := a.db.QueryContext(ctx, `
	...
	`)
	if err != nil {
		return nil, fmt.Errorf("mariadb duplicate indexes: %w", err)
	}
	defer rows.Close()
...
func (a *MariaDBAdapter) GetTableStats(ctx context.Context) ([]models.TableStat, error) {
	rows, err := a.db.QueryContext(ctx, `
		SELECT
			t.TABLE_SCHEMA,
			t.TABLE_NAME,
	...
	`)
	if err != nil {
		return nil, fmt.Errorf("mariadb table stats: %w", err)
	}
	defer rows.Close()
